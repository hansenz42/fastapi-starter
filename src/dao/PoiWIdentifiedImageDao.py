...
class PoiWIdentifiedImageDao:
    ...
    async def add_identified_image(self, image_url: str, keypoints: bytes, descriptors: bytes, poi_uuid: str, image_uuid: str):
    ...
    async def get_identified_image(self, image_uuid: str) -> PoiWIdentifyImagePo:
        async with self.__session_maker() as session:
            result = await session.execute(select(PoiWIdentifyImagePo).where(PoiWIdentifyImagePo.image_uuid == image_uuid))
